...
router.get('/', authenticateToken, async (req: AuthenticatedRequest, res) => {
  try {
    ...
    let query: any = db.collection('discussions');
    ...
    let discussions = snapshot.docs.map((doc: any) => ({
      id: doc.id,
      ...doc.data(),
    ...
    if (search) {
      ...
      discussions = discussions.filter((discussion: any) => 
        discussion.title.toLowerCase().includes(searchTerm) ||
        discussion.content.toLowerCase().includes(searchTerm) ||
        discussion.tags?.some((tag: string) => tag.toLowerCase().includes(searchTerm))
      );
    }
...
router.put('/:id', authenticateToken, async (req: AuthenticatedRequest, res) => {
  try {
    ...
    await db.collection('discussions').doc(id).update(updateData);
...
router.patch('/:id/pin', authenticateToken, requireRole('teacher'), async (req: AuthenticatedRequest, res) => {
  try {
    const { id } = req.params;
...
router.patch('/:id/lock', authenticateToken, requireRole('teacher'), async (req: AuthenticatedRequest, res) => {
  try {
    const { id } = req.params;
...
// Get discussion tags
router.get('/meta/tags', authenticateToken, async (req: AuthenticatedRequest, res) => {
  try {
    const snapshot = await db.collection('discussions').get();
    ...
    snapshot.docs.forEach((doc: any) => {
      ...
      discussionTags.forEach((tag: string) => tags.add(tag));
    });
